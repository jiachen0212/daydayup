...
def localTranslationWarpFastWithStrength(srcImg, startX, startY, endX, endY, radius, strength):
    ...
    # cv2.remap()重映射: 把一幅图像内的像素点放置到另外一幅图像内的指定位置
    copyImg = cv2.remap(srcImg, UX, UY, interpolation=cv2.INTER_LINEAR)
 
    return copyImg
 
 
image = cv2.imread("./san.jpg")
...
# startX_left, startY_left, endX_left, endY_left = 138, 299, 228, 247
# startX_right, startY_right, endX_right, endY_right = 309, 291, 228, 247
startX_left, startY_left, endX_left, endY_left = 669, 963, 865, 833
startX_right, startY_right, endX_right, endY_right = 1075, 955, 865, 833
...
cv2.imwrite("./san_thin.jpg", processed_image[:, :, ::-1])

# 把瘦脸前后的图合并成gif
imageio.mimwrite('./san.gif', [image, processed_image], duration=0.35)
